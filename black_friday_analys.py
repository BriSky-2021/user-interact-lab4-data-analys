...
# user_Purchase_top10_top= user_Purchase_top10.sort_values('下单次数',ascending=False).head(10)
# fig = px.bar(user_Purchase_top10_top, x="User_ID", y="总消费额")
...
def feature_group(data, group, text):  # data:数据集，group:要分类的字段
    ...
    # 添加注释
    inner_fig.add_annotation(text="注：这里是一些说明。内圈是属性，外圈是消费总金额，鼠标悬停可查看人数", xref="paper",
...
fig6.update_layout(title="Top20商品销售额对比图(万)")
